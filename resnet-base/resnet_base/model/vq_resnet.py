...
class VQResNet(ResNet):
    def _build_model(self, x: tf.Tensor) -> tf.Tensor:
        ...
        with tf.variable_scope(self.custom_scope, auxiliary_name_scope=False):
            ...
            vq_endpoints = vq(x, n=512, alpha=1, beta=.5, gamma=0, num_splits=16, lookup_ord=1, return_endpoints=True,
            ...
            self.logger_factory.add_scalar('unused_embeddings', tf.nn.zero_fraction(access_count), log_frequency=25)
