...
class ParallelVQResNet(ResNet):
    ...
    def _build_model(self, x: tf.Tensor) -> tf.Tensor:
        ...
        x = self.__parallel_vqs(x, num_parallel=16, n=128, name='vq_post_conv1', num_splits=2)
        x = ResNet._v2_block(x, 'block1', base_depth=64, num_units=3, stride=2)  # 8x8x256
        x = self.__parallel_vqs(x, num_parallel=64, n=128, name='vq_post_block1', num_splits=2)
        x = ResNet._v2_block(x, 'block2', base_depth=128, num_units=4, stride=2)  # 4x4x512
        x = self.__parallel_vqs(x, num_parallel=128, n=128, name='vq_post_block2', num_splits=2)
