...
def graph(images, drop_prob, wd):
    ...
    with tf.variable_scope('softmax_linear') as scope:
        weights = _variable_with_weight_decay('weights', [512, data.NUM_CLASSES],
                                              stddev=1 / 512.0, wd=None)
        biases = _variable_on_cpu('biases', [data.NUM_CLASSES],
                                  tf.constant_initializer(0.0))
        softmax_linear = tf.add(tf.matmul(local4, weights), biases, name=scope.name)
        _activation_summary(softmax_linear)
    return softmax_linear, tf.nn.softmax(softmax_linear, axis=1)
